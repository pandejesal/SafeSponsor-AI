...
### SHIPPED + VERIFIED LIVE (2026-08-20, deploy `dpl_6W41FKgEpWoZJ8ngr1d3NZP6xz8J`, commit `b0cc822`)
- **P7 — Anonymous free teaser + email capture — DONE**: the free check no longer requires sign-in. A fast first-impression scan (`runTeaserScan`: ONE Gemini call, 40s timeout, Groq fallback, no video fetching) returns in seconds — the full pipeline exceeded the 50s Vercel budget on cold starts. Dual-marker 1-per-IP + 1-per-account cap (atomic check+set, rollback of BOTH on failure, reads-before-writes), per-IP rate limit, App Check exempted ONLY for anonymous teasers (signed-in paths still enforced). Score card shows "Free preview" + **email capture ("Email me the full dossier")** → new `/api/lead` endpoint (lead doc with email + target + score, `source=landing-teaser`, normalized email, tombstone-based dedup). Repeated teasers get "already used your free check"; signed-in callers on a used IP get the same 429. Tombstoned creators denied before the cap check.
- **P6 — Usage soft-limits — DONE**: `/api/analyze` now returns **402** for zero-credit/no-sub users (was a hard wall); dashboard shows a soft-limit banner ("You're out of credits — top up or upgrade") with the 3-Pack card ($19 one-time · Save 21%) and a top-up row on the credits card. **Latent bug fixed**: `isChannelAudit` was declared AFTER the quota transaction but referenced inside its Firestore callback (which runs synchronously) — every free 0-credit request crashed with a TDZ 500 instead of the intended 402. Declared above the transaction block.
- **E2E 13/13 PASS** (`.swarm/e2e/teaser-e2e.js`, production): P0 reset → T1 hero input, T2 anon teaser score card (real Gemini scan), L1/L2/L3 email capture + lead doc, T3 repeat blocked (429), T4/T5 signed-in session mint + IP-block (429), P6-3 top-up row, P6-1 402 banner, P6-2 3-pack card. N1 unit suite 14/14 PASS.

### CHORE: Next.js 16.3.1 upgrade — DONE (commits `34f187c` + `5b8bf9f`, deploy `dpl_D8VTFz3okebmBjYdTrTDrJMJbK2D`)
- **Why:** cleared the last HIGH-severity npm audit findings (postcss 8.4.x RCE via `next@15`, sharp 0.34 vuln). Upgrade pulled in `next@16.3.1`, `eslint-config-next@16.3.1`, `sharp@0.35.3`, `postcss@8.5.23` — **0 high/critical remain**; only 3 moderate left, all dev-only `firebase-tools → @google-cloud/pubsub → @opentelemetry/core` (pubsub pins `^1.30.1` in the vulnerable range; no non-breaking fix exists — accepted risk).
- **v16 breaking changes absorbed:** Turbopack is now the build default (removed the dev-only webpack DISABLE_HMR hook); `eslint` field removed from `next.config` type; `next lint` removed; ESLint flat config now imports `eslint-config-next/core-web-vitals` directly (FlatCompat legacy-extends crashed in v16); Sentry client init moved `sentry.client.config.ts → instrumentation-client.ts` (Turbopack no longer loads the legacy file); tsconfig/next-env auto-migrations applied by the installer.
- **react-hooks v7 `set-state-in-effect` rule** flags 8 pre-existing legitimate patterns (hydration mount-guards, one-shot URL/localStorage/matchMedia reads, navigation reset, intentional App Check full-reload redirect). Each got a documented `eslint-disable` — no risky refactors. Lint clean, `tsc --noEmit` clean, all 4 suites green (N1 14/14, M1 18/18, M4 20/20, M5 15/15 = 67/67), E2E 13/13 PASS re-verified on prod after deploy.
- **Deploy gotcha:** `output: 'standalone'` (added for self-hosting) broke the Vercel cloud build with `ENOENT .next/next-server.js.nft.json` (standalone + Sentry instrumentation adapter vs Turbopack nft tracing, nextjs issue #96646); local `vercel build` also hit the flaky `NEXT_MISSING_LAMBDA` grouping bug. Fix: removed `output: 'standalone'` — Vercel builds its own lambdas from the standard output. Cloud deploy READY, health 200 `{"ok":true,"db":"ok","paymentsMode":"test"}`.
...
- **GROQ fallback for teaser scans**: `GROQ_API_KEY` is empty at runtime (Vercel stored value blank) and local env files carry placeholder keys — the Groq fallback path is untested in prod. Needs a real key to activate.
- **Gemini free-tier quota**: `gemini-3.6-flash` is on a 20 req/day free tier — fine for current traffic, but teaser scans will 429 (RESOURCE_EXHAUSTED) once the day's quota is spent; upgrade to pay-as-you-go before launch traffic.
